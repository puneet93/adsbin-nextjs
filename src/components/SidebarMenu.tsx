...
import { CircleHelp, Cog, DollarSign, EyeIcon, FilmIcon, FlipHorizontal, HomeIcon, MenuIcon, ThumbsUp, TrendingUpIcon, UsersIcon } from "lucide-react";
...
export default function Sidebar(){
    ...
    return(
        <>
            <Button onClick={() => setShow(!show)} className="md::hidden fixed z-20 top-10 left-3.5 pb-1" variant={'link'} size={'icon'}><MenuIcon className="!w-8 !h-8" color="#fff" /></Button>
            <div className={`fixed z-10 top-0 ${show ? 'left-0' : 'md:left-0 -left-80'} transition-all w-80 mt-[112px] bottom-0 border-r border-adsbin-grey-100 overflow-auto flex bg-white flex-col`}>
